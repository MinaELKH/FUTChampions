...
<button  value="${player.positionInStade}" onclick="ShowModalChoixPlayer(event)" name_form="btnEdit" class="  text-red-800 material-symbols-outlined cursor-pointer text-white hover:text-red-400 text-xl font-semibold ">
swap_horiz
</button>
  </div>
     <div class="Score">
...
function getPlayer(id) {
  console.log(" deletedPlayer  idplayer   :");
  console.log(id);
...
function ShowModalChoixPlayer(event ) {
 ...
 // divParent = document.getElementById(event.target.value);
...
function positionnerPlayer(idvalue , positionARemplir , event) {
  ...
  console.log(" positionnerPlayer: "  + positionARemplir)  ;  // pourquoi undefined
...
function ChangePositionPlayer( idplayer  , event  ) {
  ShowModalChoixPlayer(event);

 


  let pl = getPlayer(idplayer);
  console.log("joueur remplace: " + pl) ; 
  pl.isActif = false;
  pl.positionInStade = "";
  players.push(pl);
  ...
  codehtml =playerCodeHtml(pl);  // qjout nouveau badget 
  divPosition.innerHTML = codehtml;
